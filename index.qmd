...
<br>
A Matriz de Correlação abaixo foi confeccionada com a finalidade de evidenciar possíveis relações entre as variáveis coletadas na pesquisa:
<br>
...
cor_stretch <- cor_mat |>
  corrr::stretch() |>
  dplyr::mutate(
    x = factor(x, levels = numeric_vars),
    y = factor(y, levels = numeric_vars),
    r2 = r^2
  ) |>
...
Na regressão linear simples, **a espessura do reto femoral (RFE) mostrou associação positiva e estatisticamente significativa com a distância de marcha** (β = 221,59; p = 0,0096). 
Em termos práticos, maior espessura muscular esteve relacionada a melhor capacidade funcional: *para cada aumento de 0,1 unidade de espessura, estima-se ganho médio de aproximadamente 22 metros na marcha*. 
O modelo apresentou bom ajuste para esta amostra, explicando cerca de 50,5% da variação da distância percorrida (R² = 0,505; R² ajustado = 0,456), com significância global (F(1,10) = 10,22; p = 0,0095). 
O erro padrão residual foi de 19,54 metros, indicando a variabilidade esperada entre valores observados e previstos. 
Entretanto, o intercepto não foi significativo (p = 0,434) e sua interpretação clínica isolada é limitada.
