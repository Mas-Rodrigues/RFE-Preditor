...
No período de **`r min(data$DATA)`** a **`r max(data$DATA)`**, foram realizados **`r nrow(data)`** registros de acompanhamento de reabilitação cardiopulmonar, cujas métricas (variáveis) estão evidenciadas na tabela abaixo:
<br>
...
print(
  ggplot(cor_stretch, aes(x = x, y = y, fill = r)) +
    geom_tile() +
    scale_fill_gradient2(low = "blue", high = "red", mid = "white", midpoint = 0) +
    labs(title = "Matriz de calor das correlações", x = "", y = "") +
    theme_minimal()
)
```
...
## Regressão Linear
```{r}
model1 <- lm(MARCHA ~ `RFE (espessura)`, data = data)
summary(model1)
```
